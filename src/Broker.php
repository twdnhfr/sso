...
class Broker
{
     ...
     */
    public function generateToken($cookieBased)
    {
        if (isset($this->token)) {
            return;
        }
        ...
        if ($cookieBased) {
            setcookie($this->getCookieName(), $this->token, time() + $this->cookie_lifetime, '/', '.' . getenv('HTTP_HOST'));
     ...
     * @return string
     */
    public function getAttachUrl($params = [], $cookieBased = null)
    {
    ...
    public function attach($returnUrl = null, $cookieBased = true)
    {
        ...
        if ($cookieBased) {
            $params = array_merge($params, ['cookie' => true]);
        }

        $url = $this->getAttachUrl($params, $cookieBased);
